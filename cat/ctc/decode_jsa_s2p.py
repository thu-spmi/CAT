...
def dataserver(args, q: mp.Queue):
    ...
    t_beg = time.time()
    for batch in tqdm(
        testloader, desc="CTC based S2P decode", total=len(testloader), leave=False
    ):
        for k in batch:
...
def worker(
    ...
    phone_tokenizer = tknz.load(args.phone_tokenizer)

    labels = [""] * phone_tokenizer.vocab_size
    searcher = CTCDecoder(
        labels,
    ...
    with torch.no_grad():
        while True:
            ...
            x = x.to(device)
            key = key[0]
            logits, olens = model.s2p_encoder(x, x_len)
            ...
            if args.do_sample:
                # sample n phoneme sentence for each utterence
                if args.sample_each_utt:
                    ...
                    while len(sample_list) < args.n_samples:
                        beam_results, out_lens = model._sample(logits.exp(), olens, n_samples=1)
                    ...
                    sample_results, sample_lens = model._sample(logits.exp(), olens, n_samples=args.n_samples)
                    N, K, T = sample_results.shape
            ...
            q_out.put(
                (
                    ...
                    {
                        bid: (score.item(), 
                              '' if len(hypo[:_len].tolist())==0 else phone_tokenizer.decode(hypo[:_len].tolist())
                              )
...
def _parser():
    ...
    parser.add_argument("--sample_each_utt", type=bool, default=False, help="sample n phoneme sentence for each utterence.")
    return parser
