    ...
    with torch.no_grad():
        while True:
            ...
            if args.streaming:
                ...
                logits, olens = model.bf_chunk_infer(x, x_len)
            else:
                # logits, olens = model.encoder(x, x_len)
                
                # x = x[...,0]
                # samples, flens = model.stft(x, x_len)
                # input_power = samples[..., 0] ** 2 + samples[..., 1] ** 2
                # input_amp = torch.sqrt(torch.clamp(input_power, min=1.0e-10))
                # feats, _ = model.logmel(input_amp, flens)      
                feats, flens = model.beamforming(x, x_len)
                
                logits, olens = model.encoder(feats, flens)
