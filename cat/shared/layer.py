...
class BertSelfAttention(nn.Module):
    """
        Bert Self Attention implement
        Based on: https://github.com/huggingface/transformers/blob/main/src/transformers/models/bert/modeling_bert.py#L244
    """
    def __init__(self,
                 hdim,
                 num_heads,
        ...
        if hdim % num_heads != 0:
            raise ValueError(
                f"The hidden size ({hdim}) is not a multiple of the number of attention "
                f"heads ({num_heads})"
            )
        self.num_heads = num_heads
        self.attention_head_size = int(hdim / num_heads)
        self.all_head_size = self.num_heads * self.attention_head_size

        self.query = nn.Linear(hdim, self.all_head_size)
        self.key = nn.Linear(hdim, self.all_head_size)
        self.value = nn.Linear(hdim, self.all_head_size)
        ...
        self.output_dense = nn.Linear(hdim, hdim)
        self.LayerNorm = nn.LayerNorm(hdim, eps=layer_norm_eps)
    ...
    def forward(
        ...
        attention_output = self.LayerNorm(
                                self.dropout(
                                    self.output_dense(context_layer)) + hidden_states)
...
class BertLayer(nn.Module):
    def __init__(self,
                 ...
                 position_embedding_type: Literal["relative_key", "relative_key_query", "none"] = "relative_key",
                 add_cross_attention: bool = False,
                 layer_norm_eps: float = 1e-12
                 ):
        super().__init__()
        self.attention = BertSelfAttention(hdim, num_heads, hidden_dropout_prob, 
                                           max_position_embeddings, attention_probs_dropout_prob,
                                           is_decoder, position_embedding_type, layer_norm_eps)
        self.is_decoder = is_decoder
        self.add_cross_attention = add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = BertSelfAttention(hdim, num_heads, hidden_dropout_prob, 
                                                    max_position_embeddings, attention_probs_dropout_prob,
                                                    is_decoder, position_embedding_type="absolute", layer_norm_eps=layer_norm_eps)
        self.intermediate_dense = nn.Linear(hdim, intermediate_size)
        self.output_dense = nn.Linear(intermediate_size, hdim)
        self.LayerNorm = nn.LayerNorm(hdim, eps=layer_norm_eps)
        self.dropout = nn.Dropout(hidden_dropout_prob)
    ...
    def forward(
        ...
        if self.is_decoder and encoder_hidden_states is not None:
            if not hasattr(self, "crossattention"):
                raise ValueError(
                    ...
                    " by setting `add_cross_attention=True`"
                )
        ...
        layer_output = self.LayerNorm(self.dropout(self.output_dense(intermediate_output)) + attention_output)

        return (layer_output,) + (present_key_value,) if self.is_decoder else (layer_output,)
...
class BertEncoder(nn.Module):
    def __init__(self,
                 ...
                 **bert_layer_kwargs):
        super().__init__()
        self.layer = nn.ModuleList([BertLayer(**bert_layer_kwargs) for _ in range(num_hidden_layers)])
