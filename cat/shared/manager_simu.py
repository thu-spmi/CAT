...
"""training/evaluating manager
   用于simu任务, 输出ASR-loss,simu-loss, 并且具有分块加载的功能
"""
...
def train(
    ...
    def _go_step(minibatch) -> Tuple[torch.Tensor, int]:
        ...
        with autocast(enabled=use_amp):
            if hook_func is None:
                loss, utt_loss, chunk_loss, simu_loss = model(*minibatch)
            else:
            ...
            simu_loss = simu_loss.detach().clone()
        ...
        # return for logging
        return raw_loss, feats.size(0),utt_loss, chunk_loss, simu_loss
    ...
    for i, minibatch in enumerate(trainloader):
        ...
        # 或者可以改为，打开梯度累积操作 flag，统计n_batch，当其 >= batch_size的时候自动进行一次梯度更新
            local_loss, local_bs, utt_loss, chunk_loss, simu_loss = _go_step(minibatch)
            accum_loss += local_loss * local_bs
            accum_utt_loss += utt_loss * local_bs
            accum_chunk_loss += chunk_loss * local_bs
            accum_simu_loss += simu_loss * local_bs
            n_batch += local_bs
            ...
            item_simu_loss = (accum_simu_loss / n_batch).item()

            # update tensorboard
            if args.rank == 0:
                manager.writer.add_scalar("loss_train/total_with_simu", item_loss, manager.step)
                ...
                manager.writer.add_scalar("loss_train/total", item_utt_loss + item_chunk_loss, manager.step)
                manager.writer.add_scalar("loss_train/simu", item_simu_loss, manager.step)
...
def evaluate(
    ...
    for minibatch in tqdm(
        ...
        total_simu_loss += simu_loss.float() * feats.size(0)
    ...
    dist.all_reduce(total_simu_loss, dist.ReduceOp.SUM)
    ...
    avg_simu_loss = (total_simu_loss / cnt_seq).item()

    if args.rank == 0:
        manager.writer.add_scalar("loss_dev/total_with_simu_loss", avg_loss, manager.step)
        ...
        manager.writer.add_scalar("loss_dev/simu_loss", avg_simu_loss, manager.step)
        manager.writer.add_scalar("loss_dev/total_loss", avg_utt_loss + avg_chunk_loss, manager.step)
    return avg_loss
