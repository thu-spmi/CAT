...
# see `eesen/asr_egs/wsj/local/wsj_expand_vocab.sh`

if [ $# -ne 3 ]; then
  echo "usage: local/expand_vocab.sh <corpus-dir> <dict-src-dir> <dict-larger-dir>"
  echo "e.g.: local/expand_vocab.sh /foo/bar/WSJ/13-32.1/ data/local/lang_char data/local/lang_char_larger"
  exit 1;
fi
