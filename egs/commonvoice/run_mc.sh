...
if [ $NODE == 0 ]; then
    ...
    if [ $stage -le 5 ] && [ $stop_stage -ge 5 ]; then
        ...
        mkdir -p $ark_dir
        for usg in train dev test; do
            for x in ${lang[@]} sp; do
                dir=data/${usg}
                scp=$ark_dir/${usg}_${x}.scp
                ark=$ark_dir/${usg}_${x}.ark
                if [ $x = "sp" ]; then
                    if [ $usg != "test" ]; then
                        dir=${dir}_${x}
                        scp=$ark_dir/${usg}.scp
                ...
                feats_x="ark,s,cs:apply-cmvn --norm-vars=true --utt2spk=ark:$dir/utt2spk scp:$dir/cmvn.scp scp:$dir/feats.scp ark:- | add-deltas ark:- ark:- | subsample-feats --n=3 ark:- ark:- |"
                copy-feats "$feats_x" "ark,scp:$ark,$scp" || exit 1
            done
        done
