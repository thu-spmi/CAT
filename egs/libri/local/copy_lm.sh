...
if [ $# -ne "2" ]; then
  echo "Usage: $0 <src_dir> <download_dir>"
  exit 1
fi
...
function check_and_copy () {
  [[ $# -eq 1 ]] || { echo "check_and_copy() expects exactly one argument!"; return 1; }
  fname=$1
  echo "copy file '$fname' into '$dst_dir'..."
