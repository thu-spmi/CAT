...
if [ $stage -le 6 ]; then
  ...
  for j in $(seq 1 $nj); do
    ...
    inp_file=/mnt/workspace/pengwenjie/returnn/2019-lm-transformers/librispeech/exp/input_${j}.txt
    cp ${adir}.${j}/words_text.nouttid $inp_file
    out_file=/mnt/workspace/pengwenjie/returnn/2019-lm-transformers/librispeech/exp/output_${j}.txt
    desconfig=${adir}.${j}/transformer-lm.config
    cp $srcconfig $desconfig
    echo "data_files = { 'test': '$inp_file' }" >> $desconfig
    echo "eval_output_file_per_seq = '$out_file' " >> $desconfig
    python $run_script --config $desconfig --load $nnmd || exit 1;
    paste -d " " ${adir}.${j}/words_text.uttid $out_file > ${adir}.${j}/lmwt.nn
  done
