...
if [ $NODE == 0 ]; then
    ...
    if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
        ...
        local/prepare_bpe_dict_ctc.sh $data_tr/text data/local/dict_phn data/local/dict_bpe || exit 1;

        ctc-crf/ctc_compile_dict_token.sh data/local/dict_bpe data/local/lang_bpe_tmp data/lang_bpe || exit 1;
        local/format_lms.sh --src-dir data/lang_bpe data/local/lm || exit 1

        # Create ConstArpaLm format language model for full 3-gram and 4-gram LMs
        # 3 index <UNK> in the wordlist
        echo "3" > data/lang_bpe/oov.int
        utils/build_const_arpa_lm.sh data/local/lm/lm_tglarge.arpa.gz \
          data/lang_bpe data/lang_bpe_tglarge || exit 1
        utils/build_const_arpa_lm.sh data/local/lm/lm_fglarge.arpa.gz \
          data/lang_bpe data/lang_bpe_fglarge || exit 1

        langdir=data/lang_bpe_tgsmall
    ...
    if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
        ...
        python3 ctc-crf/prep_ctc_trans.py data/lang_bpe/lexicon_numbers.txt $data_tr/text "<UNK>" > $data_tr/text_number || exit 1
        python3 ctc-crf/prep_ctc_trans.py data/lang_bpe/lexicon_numbers.txt $data_cv/text "<UNK>" > $data_cv/text_number || exit 1
        ...
        python3 ctc-crf/ctc_token_fst_corrected.py den data/lang_bpe/tokens.txt | fstcompile | fstarcsort --sort_type=olabel > data/den_meta/T_den.fst || exit 1
