...
for f in $data/stm $data/glm $lang/words.txt \
...
if [ $stage -le 0 ]; then
  for wip in $(echo $word_ins_penalty | sed 's/,/ /g'); do
    $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/get_ctm.LMWT.${wip}.log \
      ...
      lattice-scale --acoustic-scale=10.0 --lm-scale=LMWT "ark:gunzip -c $dir/lat.*.gz|" ark:- \| \
...
if [ $stage -le 1 ]; then
  ...
  for x in $dir/score_*/$name.ctm; do
    ...
    grep -v -- '-$' > $x;
    python local/map_acronyms_ctm.py -i $x -o $x.mapped -M data/local/dict_phn/acronyms.map
    cp $x $x.bk
    mv $x.mapped $x
  done
fi
