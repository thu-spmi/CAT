...
if [ $NODE == 0 ]; then
    if [ $stage -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        local/swbd1_prepare_bpe_dict.sh || exit 1


        # Compile the lexicon and token FSTs
        ctc-crf/ctc_compile_dict_token.sh data/local/dict_bpe data/local/lang_bpe_tmp data/lang_bpe || exit 1;

        # Train and compile LMs.
        local/swbd1_train_lms.sh data/local/train/text data/local/dict_bpe/lexicon.txt data/local/lm $fisher_dirs || exit 1;
        ...
        utils/format_lm_sri.sh --srilm-opts "$srilm_opts" \
                            data/lang_bpe $LM data/local/dict_bpe/lexicon.txt data/lang_bpe_sw1_tg || exit 1;
        old_lang=data/lang_bpe
        new_lang=data/lang_bpe_sw1_fsh_fg
        mkdir -p $new_lang
    ...
    if [ $stage -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ctc-crf/prep_ctc_trans.py data/lang_bpe/lexicon_numbers.txt $data_tr/text "<unk>" > $data_tr/text_number || exit 1;
        ctc-crf/prep_ctc_trans.py data/lang_bpe/lexicon_numbers.txt $data_cv/text "<unk>" > $data_cv/text_number || exit 1;
        echo "convert text_number finished"
    
        ## prepare denominator
        ctc-crf/prep_ctc_trans.py data/lang_bpe/lexicon_numbers.txt data/train_nodup/text "<unk>" > data/train_nodup/text_number || exit 1;
        ...
        ctc-crf/ctc_token_fst_corrected.py den data/lang_bpe/tokens.txt | fstcompile | fstarcsort --sort_type=olabel > data/den_meta/T_den.fst || exit 1;
...
if [ $stage -le 8 ] && [ ${stop_stage} -ge 8 ]; then
    ...
    ctc-crf/decode.sh --stage 1 \
        ...
        data/lang_bpe_sw1_tg data/eval2000 data/all_ark/eval2000.scp $dir/decode_eval2000_sw1_tg

    steps/lmrescore_const_arpa.sh --cmd "$cmd" data/lang_bpe_sw1_{tg,fsh_fg} data/eval2000 $dir/decode_eval2000_sw1_{tg,fsh_fg}  || exit 1;
fi
