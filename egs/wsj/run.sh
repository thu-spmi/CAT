...
if [ $stage -le 8 ]; then
  ...
  for set in dev93 eval92; do
    ...
    ./local/lmrescore_const_arpa.sh --cmd "$deccode_cmd" data/lang_phn_test_bd_{tgpr,fgconst} data/test_${set} exp/decode_${set}/lattice_bd_{tgpr,fgconst} || exit 1;
    mkdir -p exp/decode_${set}/lattice_tg
    ./local/lmrescore.sh --cmd "$decode_cmd" data/lang_phn_test_{tgpr,tg} data/test_${set} exp/decode_${set}/lattice_{tgpr,tg} || exit 1;
done
fi
