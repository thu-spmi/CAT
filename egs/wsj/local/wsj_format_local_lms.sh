...
[ ! -d data/lang${lang_suffix}_larger ] &&\
  echo "Expect data/local/lang${lang_suffix} to exist" && exit 1;
...
for d in data/lang${lang_suffix}_test_bd_{tg,tgpr,tgconst,fg,fgpr,fgconst}; do
  rm -r $d 2>/dev/null
  cp -r data/lang${lang_suffix}_larger $d
done

lang=data/lang${lang_suffix}_larger

# Parameters needed for ConstArpaLm.
unk=`grep "<SPOKEN_NOISE>" $lang/words.txt | awk '{print $2}'`
