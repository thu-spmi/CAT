...
grep -v 'u:7\|ttss_h5\|eI7\|i:7' $source_dir/lexicon.txt > $dict_dir/lexicon.txt
...
grep -v 'u:7\|ttss_h5\|eI7\|i:7' $source_dir/lexicon.txt | grep -v -F -f $dict_dir/silence_phones.txt | \
    perl -ane 'print join("\n", @F[1..$#F]) . "\n"; '  | \
